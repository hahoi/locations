...
function delImg(key, index) {
  // console.log(park.廁所[key].照片[index], key, index);
  // 刪除storage上的照片
  storageImgDelete(park.廁所[key].照片[index].findKey);
  park.廁所[key].照片.splice(index, 1);
}
...
async function saveImageMessage(fileName, file) {
  // console.log(file);
  const message = park.廁所[data.key];
  ...
  try {
    ...
    // console.log(message, data.key, park.廁所[data.key]);
    // 很多張照片，用array
    const image = {
      url: publicImageUrl,
      findKey: filePath,
