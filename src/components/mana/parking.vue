...
function delImg(key, index) {
  // console.log(park.停車場[key].照片[index], key, index);
  // 刪除storage上的照片
  storageImgDelete(park.停車場[key].照片[index].findKey);
  park.停車場[key].照片.splice(index, 1);
}
...
async function saveImageMessage(fileName, file) {
  // console.log(file);
  const message = park.停車場[data.key];
