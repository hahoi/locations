...
function appendItem() {
  ...
  // console.log(park.附近美食);
}

// 刪除item 資料
function delItem(index) {
  // 刪除storage上的照片
  if (park.附近美食[index].照片) {
    park.附近美食[index].照片.forEach((element) => {
...
function delImg(key, index) {
  // console.log(park.附近美食[key].照片[index], key, index);
  // 刪除storage上的照片
  storageImgDelete(park.附近美食[key].照片[index].findKey);
  park.附近美食[key].照片.splice(index, 1);
}
...
async function saveImageMessage(fileName, file) {
  // console.log(file);
  const message = park.附近美食[data.key];
  ...
  try {
    ...
    // console.log(message, data.key, park.附近美食[data.key]);
    // 很多張照片，用array
    const image = {
      url: publicImageUrl,
      findKey: filePath,
